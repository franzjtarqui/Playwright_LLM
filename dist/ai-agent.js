...
Instruction: "Verify title 'Dashboard', click on 'Settings', then click on 'Users'"
{
  ...
  "needsVerification": true
     ...
     * });
     */
    async executeFlow({ url, steps, stopOnError = true, delayBetweenSteps = 2000, analysisMode, enableTracing = false, generateReport = false, reportDir = './test-results' }) {
        if (!this.page)
            throw new Error('Agente no inicializado. Llama a initialize() primero.');
        // Usar el modo pasado por parámetro o el configurado en la instancia
        if (analysisMode)
     ...
     */
    async captureStepScreenshot(stepNumber, success, reportDir) {
        if (!this.page)
            return '';
     ...
     */
    async generateHTMLReport(result, reportDir) {
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
...
<body>
  <div class="container">
     ...
     */
    generateSlackPayload(result, options) {
